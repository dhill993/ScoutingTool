...
# Position-specific metric categories (for Key Attributes replacement)
POSITION_METRIC_CATEGORIES = {
    'FB': {
        'Passing Quality': ['Passing%', 'Pr. Pass%', 'OP Passes Into Box'],
        'Regains': ['Defensive Regains', 'Aerial Win%', 'Ball Recoveries', 'Dribbles Stopped%'],
        'Creating': ['xG Assisted', 'OP F3 Passes', 'OP Key Passes', 'xGBuildup'],
        'Turnovers': ['Fouls Won']  # Using available metric as proxy
    },
    'CB': {
        'Passing Quality': ['Passing%', 'Pr. Pass%', 'Pass Forward%'],
        'Regains': ['Defensive Regains', 'Aerial Win%', 'Ball Recoveries', 'Dribbles Stopped%'],
        'Creating': ['xG Assisted', 'OP F3 Passes', 'OP Key Passes', 'xGBuildup'],
        'Turnovers': ['Fouls Won']  # Using available metric as proxy
    },
    'DM': {
        'Passing Quality': ['Passing%', 'Pr. Pass%', 'OP F3 Passes'],
        'Regains': ['Defensive Regains', 'Aerial Win%', 'Ball Recoveries', 'Dribbles Stopped%'],
        'Creating': ['xG Assisted', 'OP F3 Passes', 'OP Key Passes', 'xGBuildup'],
        'Turnovers': ['Fouls Won']  # Using available metric as proxy
    },
    ...
    'AM': {
        'Finishing': ['xG', 'Shots', 'Shooting%', 'Shot Touch %'],
        'Creating': ['xG Assisted', 'PINTIN', 'OP Key Passes'],
        'Turnovers': ['Fouls Won'],  # Using available metric as proxy
        'Pressing': ['Pressures', 'Counterpressures', 'Aggressive Actions']
    },
    'WF': {
        'Finishing': ['xG', 'Shots', 'Shooting%', 'Shot Touch %'],
        'Creating': ['xG Assisted', 'PINTIN', 'OP Key Passes'],
    ...
    'GK': {
        'Shot Stopping': ['GSAA', 'xSv%', 'Save%'],
        'Passing': ['Passing%', 'Pr. Pass%'],
        'Sweeping': ['GK Aggressive Dist.'],
        'Cross Claiming': ['Claims%']
    }
}
...
def calculate_attribute_scores(player_data_pct, position):
    """Calculate category scores from percentiles for position-specific metrics"""
    if position not in POSITION_METRIC_CATEGORIES:
        return {}
    
    scores = {}
    for category_name, metrics in POSITION_METRIC_CATEGORIES[position].items():
        available_values = []
        for metric in metrics:
            if metric in player_data_pct.index and not pd.isna(player_data_pct[metric]):
                available_values.append(float(player_data_pct[metric]))
        
        if available_values:
            scores[category_name] = round(np.mean(available_values))
        else:
...
def main():
    ...
    # Stage 1: Load competitions list only (very fast)
    with st.spinner("Loading competitions..."):
        comps_df = fetch_competitions_list()
    
    if comps_df.empty:
        st.error("No competitions loaded")
        st.stop()
    
    # Step 1: Select Competition
    comp_displays = comps_df.apply(lambda x: f"{x['competition_name']} ({x['country_name']})", axis=1).tolist()
    selected_comp_display = st.sidebar.selectbox("1️⃣ Select Competition", ["Select Competition..."] + sorted(set(comp_displays)))
    
    if selected_comp_display == "Select Competition...":
        st.info("👈 Start by selecting a competition")
        st.stop()
    
    # Get all seasons for selected competition
    comp_name = selected_comp_display.split(" (")[0]
    comp_seasons = comps_df[comps_df['competition_name'] == comp_name]
    
    # Step 2: Select Season
    season_displays = comp_seasons['season_name'].unique().tolist()
    selected_season = st.sidebar.selectbox("2️⃣ Select Season", ["Select Season..."] + sorted(season_displays, reverse=True))
    ...
    comp_season_row = comp_seasons[comp_seasons['season_name'] == selected_season].iloc[0]
    comp_id = comp_season_row['competition_id']
    season_id = comp_season_row['season_id']
    
    # Stage 2: Load data for selected competition/season only (on demand)
    with st.spinner(f"Loading {comp_name} - {selected_season} data..."):
    ...
    all_players = sorted(df_comp['Name'].dropna().unique().tolist())
    selected_player = st.sidebar.selectbox("3️⃣ Select Player", ["Select Player..."] + all_players)
    ...
    # Step 4: Select Position Group
    selected_position = st.sidebar.selectbox(
        "4️⃣ Select Position Group", 
        ['GK', 'CB', 'FB', 'DM', 'CM', 'AM', 'WF', 'CF'], 
        index=['GK', 'CB', 'FB', 'DM', 'CM', 'AM', 'WF', 'CF'].index(player_position) if player_position in ['GK', 'CB', 'FB', 'DM', 'CM', 'AM', 'WF', 'CF'] else 4
    )
    ...
    with tab1:
        ...
        # Percentile Chart with Profile Selection
    ...
    with tab2:
        ...
        with col1:
            ...
            # Competition 1
            comp1_displays = comps_df.apply(lambda x: f"{x['competition_name']} ({x['country_name']})", axis=1).tolist()
            sel_comp1_display = st.selectbox("Competition 1", ["Select..."] + sorted(set(comp1_displays)), key="c1")
            
            if sel_comp1_display != "Select...":
                comp1_name = sel_comp1_display.split(" (")[0]
                comp1_seasons = comps_df[comps_df['competition_name'] == comp1_name]
                season1_displays = comp1_seasons['season_name'].unique().tolist()
                sel_season1 = st.selectbox("Season 1", ["Select..."] + sorted(season1_displays, reverse=True), key="s1")
                
                if sel_season1 != "Select...":
                    c1_row = comp1_seasons[comp1_seasons['season_name'] == sel_season1].iloc[0]
        ...
        with col2:
            ...
            # Competition 2
            comp2_displays = comps_df.apply(lambda x: f"{x['competition_name']} ({x['country_name']})", axis=1).tolist()
            sel_comp2_display = st.selectbox("Competition 2", ["Select..."] + sorted(set(comp2_displays)), key="c2")
            
            if sel_comp2_display != "Select...":
                comp2_name = sel_comp2_display.split(" (")[0]
                comp2_seasons = comps_df[comps_df['competition_name'] == comp2_name]
                season2_displays = comp2_seasons['season_name'].unique().tolist()
                sel_season2 = st.selectbox("Season 2", ["Select..."] + sorted(season2_displays, reverse=True), key="s2")
                
                if sel_season2 != "Select...":
                    c2_row = comp2_seasons[comp2_seasons['season_name'] == sel_season2].iloc[0]
    ...
    with tab3:
        ...
        st.subheader(f"Top {selected_position} Players - {comp_name} {selected_season}")
        ...
        if leaderboard_data:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"{selected_position}_leaderboard_{comp_name}_{selected_season}.csv",
                mime="text/csv"
            )
        else:
